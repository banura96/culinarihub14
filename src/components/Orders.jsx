...
export default function () {
  ...
  return (
    <>
      <EventsNavigation hidden userRoles={data.user.userRole} />
      <div className="p-3">
        <nav>
          <ul className="n-barc">
            <li>
            ...
            {data.user.userRole.find((role) => role === "ADMIN") && (
              <li>
                <span
...
export async function loadingLogedInUserData() {
  ...
  let resCustomerData = await customerData.json();
  if (!customerData.ok) {
    if (customerData.message === "Unauthorized") {
      localStorage.clear();
